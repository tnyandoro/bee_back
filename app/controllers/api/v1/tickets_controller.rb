...
def create
  ...
  unless organization
    return render json: { errors: ["Organization not found"] }, status: :not_found
  end

  # Adjust params for enums
  ticket_params_adjusted = ticket_params.to_h
  if ticket_params_adjusted["urgency"]
    ticket_params_adjusted["urgency"] = ticket_params_adjusted["urgency"].downcase
  end
  if ticket_params_adjusted["impact"]
    ticket_params_adjusted["impact"] = ticket_params_adjusted["impact"].downcase
  end
  if ticket_params_adjusted["priority"]
    ticket_params_adjusted["priority"] = ticket_params_adjusted["priority"].downcase
  end

  # Validate category
  unless VALID_CATEGORIES.include?(ticket_params_adjusted["category"])
    return render json: {
      errors: ["Invalid category. Allowed values are: #{VALID_CATEGORIES.join(', ')}"]
    }, status: :unprocessable_entity
  end

  @ticket = organization.tickets.new(ticket_params_adjusted)

  if @ticket.assignee_id.present? && @ticket.team_id.present?
    assignee = User.find_by(id: @ticket.assignee_id, team_id: @ticket.team_id)
    unless assignee
      return render json: {
        errors: ["Assignee does not belong to the selected team"]
      }, status: :unprocessable_entity
    end
  end
  ...
  if @ticket.save
    ...
    Rails.logger.error "Ticket validation failed: #{@ticket.errors.full_messages}"
    render json: {
      errors: @ticket.errors.full_messages,
      details: @ticket.errors.details
    }, status: :unprocessable_entity
