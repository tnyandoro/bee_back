...
Team.where('id > ?', 6).destroy_all
User.where('id > ?', 13).destroy_all
Organization.where('id > ?', 2).destroy_all

ActiveRecord::Base.connection.execute("ALTER SEQUENCE teams_id_seq RESTART WITH 7")
ActiveRecord::Base.connection.execute("ALTER SEQUENCE users_id_seq RESTART WITH 14")
ActiveRecord::Base.connection.execute("ALTER SEQUENCE organizations_id_seq RESTART WITH 3")

# Create organizations
organizations = []

(1..3).each do |org|
  #organizations << Organization.find_by(subdomain: "organization-#{org}")
  organizations << Organization.create!(
    name: "Organization #{org}",
    subdomain: "organization-#{org}",
    email: "contact@organization-#{org}.com",
...
organizations.each_with_index do |org_data, org_index|
  puts org_data.name

  #org_team = Team.find_by(name: "Support Team #{org_index}")
  org_team = Team.create!(
    name: "Support Team #{org_index}",
    organization: org_data
  )

  users = []
  (1..10000).each do |user_index|
    users << User.create!(
      name: "User #{user_index}.#{org_index}",
      email: "user-#{user_index}@organization-#{org_index}.com",
      username: "user-#{user_index}@organization-#{org_index}.com",
      password: "password123",
      password_confirmation: "password123",
      role: :domain_admin,
      organization: org_data,
      auth_token: "user_token_#{user_index}",
      team: org_team
    )

    puts users.length if users.length % 500 == 0
  end
